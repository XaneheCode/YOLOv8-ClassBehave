...
def result_to_detections(result: Any, allowed_labels: set[str] | None = None) -> list[Detection]:
...
class YoloDetector:
    ...
    def detect(self, frame: np.ndarray) -> list[Detection]:
        ...
        if not results:
            return []
        return result_to_detections(results[0], allowed_labels=self.allowed_labels)
